...
/* ─── Export System ──────────────────────────────────────── */
function calculateStats(promptsList) {
  const total = promptsList.length;
  let ratingSum = 0;
  let ratedCount = 0;
  const modelCounts = {};

  promptsList.forEach(function (p) {
    if (p.ratings && p.ratings.average > 0) {
      ratingSum += p.ratings.average;
      ratedCount++;
    }
    const model = (p.metadata && p.metadata.model) ? p.metadata.model : "Legacy/Unspecified";
    modelCounts[model] = (modelCounts[model] || 0) + 1;
  });

  const avgRating = ratedCount > 0 ? parseFloat((ratingSum / ratedCount).toFixed(2)) : 0;
...
function exportLibrary() {
  try {
    const backupPrompts = prompts.map(function (p) {
      const clone = JSON.parse(JSON.stringify(p));
      clone.notes = loadNotes(p.id);
      return clone;
    });
    ...
    const exportData = {
      ...
      stats: calculateStats(backupPrompts),
      prompts: backupPrompts
    };

    const jsonString = JSON.stringify(exportData, null, 2);
    JSON.parse(jsonString); // parse sanity check
    ...
    a.href = url;
    a.download = "prompt_library_backup_" + formattedDate + ".json";
...
/* ─── Import System with Error Recovery ────────────────── */
...
function rollbackDatabase() {
  ...
  prompts = databaseBackup.prompts;
  savePrompts(prompts);
  try {
    localStorage.setItem(NOTES_STORAGE_KEY, JSON.stringify(databaseBackup.notes));
  } catch (e) {
    console.error("Rollback failed to restore notes to localStorage:", e);
  }
  databaseBackup = null;
  render();
}
...
function validateImportSchema(data) {
  ...
  if (!data.version) {
    throw new Error("Missing format version number.");
  }
  if (typeof data.version !== 'number' || data.version !== 1) {
    throw new Error("Unsupported import format version: " + data.version);
  }
  if (!Array.isArray(data.prompts)) {
    throw new Error("Missing prompts array.");
  }

  data.prompts.forEach(function (p, index) {
    if (!p.id || typeof p.id !== 'string') {
      throw new Error("Prompt at index " + index + " is missing a valid string ID.");
    }
    if (!p.title || typeof p.title !== 'string') {
      throw new Error("Prompt '" + (p.title || p.id) + "' is missing a title.");
    }
    if (!p.content || typeof p.content !== 'string') {
      throw new Error("Prompt '" + p.title + "' content must be a string.");
    }
    if (!p.createdAt || !isValidISO8601(p.createdAt)) {
      p.createdAt = new Date().toISOString();
    }
    if (!p.ratings || typeof p.ratings !== 'object') {
      p.ratings = { userRating: 0, average: 0, count: 0 };
    }
    
    if (p.metadata) {
      if (typeof p.metadata !== 'object') {
        throw new Error("Metadata for prompt '" + p.title + "' is invalid.");
      }
      if (!p.metadata.model || typeof p.metadata.model !== 'string') {
        p.metadata.model = "Unspecified Model";
      }
      if (!p.metadata.createdAt || !isValidISO8601(p.metadata.createdAt)) {
        p.metadata.createdAt = p.createdAt;
      }
      if (!p.metadata.updatedAt || !isValidISO8601(p.metadata.updatedAt)) {
        p.metadata.updatedAt = p.metadata.createdAt;
      }
      if (!p.metadata.tokenEstimate || typeof p.metadata.tokenEstimate !== 'object') {
        p.metadata.tokenEstimate = estimateTokens(p.content, false);
      }
    } else {
      p.metadata = trackModel("Imported Legacy Model", p.content, false);
      p.metadata.createdAt = p.createdAt;
      p.metadata.updatedAt = p.createdAt;
    }

    if (p.notes) {
      if (!Array.isArray(p.notes)) {
        throw new Error("Notes for prompt '" + p.title + "' must be an array.");
      }
      p.notes.forEach(function (n) {
        ...
        if (!n.createdAt || typeof n.createdAt !== 'number') {
...
function handleImportFileSelect(e) {
  ...
  reader.onload = function (evt) {
    try {
      ...
      const duplicates = data.prompts.filter(p => existingIds.has(p.id));

      pendingImportData = data.prompts;

      if (duplicates.length > 0) {
        duplicateCountEl.textContent = duplicates.length;
        importConflictOverlay.classList.add("active");
        confirmImportBtn.focus();
      } else {
        executeImport("merge-skip");
      }
    } catch (err) {
...
function closeImportConflictDialog() {
  importConflictOverlay.classList.remove("active");
  pendingImportData = null;
}

function executeImport(strategy) {
  ...
  try {
    ...
    if (strategy === "overwrite-all") {
      newPromptsList = pendingImportData.map(function (p) {
        const clone = JSON.parse(JSON.stringify(p));
        const notes = clone.notes || [];
        delete clone.notes;
        newNotesDict[clone.id] = notes;
        return clone;
      });
    } else {
      newPromptsList = JSON.parse(JSON.stringify(prompts));
      
      for (const k in existingNotes) {
        newNotesDict[k] = JSON.parse(JSON.stringify(existingNotes[k]));
      }

      const existingIds = new Map(newPromptsList.map(p => [p.id, p]));

      pendingImportData.forEach(function (p) {
        const clone = JSON.parse(JSON.stringify(p));
        const notes = clone.notes || [];
        delete clone.notes;

        if (existingIds.has(clone.id)) {
          if (strategy === "overwrite-conflicts") {
            const idx = newPromptsList.findIndex(x => x.id === clone.id);
            newPromptsList[idx] = clone;
            newNotesDict[clone.id] = notes;
          } else if (strategy === "keep-both") {
            const newPid = genId();
            clone.id = newPid;
            if (clone.metadata) {
              clone.metadata = JSON.parse(JSON.stringify(clone.metadata));
            }
            notes.forEach(n => {
              n.id = genId();
              n.promptId = newPid;
            });
            newPromptsList.push(clone);
            newNotesDict[newPid] = notes;
          }
        } else {
          newPromptsList.push(clone);
          newNotesDict[clone.id] = notes;
    ...
    try {
      localStorage.setItem(NOTES_STORAGE_KEY, JSON.stringify(newNotesDict));
    } catch (e) {
      if (e.name === "QuotaExceededError" || e.code === 22) {
        throw new Error("Quota exceeded error. localStorage is full.");
      }
      throw e;
    }
...
document.addEventListener("keydown", function (e) {
  if (e.key === "Escape") {
    ...
    if (importConflictOverlay.classList.contains("active")) closeImportConflictDialog();
  }
});
...
cancelImportBtn.addEventListener("click", closeImportConflictDialog);

confirmImportBtn.addEventListener("click", function () {
  const selectedStrategy = document.querySelector('input[name="conflictStrategy"]:checked').value;
  executeImport(selectedStrategy);
});

importConflictOverlay.addEventListener("click", function (e) {
  if (e.target === importConflictOverlay) closeImportConflictDialog();
});
...
function runMetadataValidationTests() {
  ...
  // Test 9: calculateStats output schema validation
  try {
    const mockPrompts = [
      { id: "1", title: "P1", content: "Word1 Word2", ratings: { average: 4.5 }, metadata: { model: "GPT-4" } },
      { id: "2", title: "P2", content: "Word3 Word4", ratings: { average: 3.5 }, metadata: { model: "Claude" } },
      { id: "3", title: "P3", content: "Word5 Word6", ratings: { average: 0 }, metadata: { model: "GPT-4" } }
    ];
    const stats = calculateStats(mockPrompts);
    assert(stats.totalPrompts === 3 && stats.averageRating === 4.0 && stats.mostUsedModel === "GPT-4", "calculateStats statistics accuracy");
  } catch (e) {
    failed++;
    console.error("✕ FAIL: calculateStats statistics accuracy threw: " + e.message);
  }

  // Test 10: validateImportSchema with valid schema structure
  try {
    const validData = {
      version: 1,
      timestamp: new Date().toISOString(),
      stats: { totalPrompts: 1, averageRating: 5, mostUsedModel: "Test" },
      prompts: [
        {
          id: "test-id-123",
          title: "Test Title",
          ...
          ratings: { userRating: 5, average: 5, count: 1 },
          metadata: {
            model: "Test Model",
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString(),
            tokenEstimate: { min: 1, max: 5, confidence: "high" }
          },
          notes: []
        }
      ]
    };
    validateImportSchema(validData);
    assert(true, "validateImportSchema valid schema parsed successfully");
  } catch (e) {
    failed++;
    console.error("✕ FAIL: validateImportSchema valid schema threw: " + e.message);
  }

  // Test 11: validateImportSchema version mismatch rejection
  try {
    const invalidData = {
    ...
    failed++;
    console.error("✕ FAIL: validateImportSchema accepted unsupported version number");
  } catch (e) {
    passed++;
    console.log("✓ PASS: validateImportSchema version mismatch rejected: " + e.message);
  }

  // Test 12: validateImportSchema missing properties validation check
  try {
    const invalidData = {
      version: 1,
      prompts: [
    ...
    validateImportSchema(invalidData);
    failed++;
    console.error("✕ FAIL: validateImportSchema accepted prompt missing content property");
  } catch (e) {
    passed++;
    console.log("✓ PASS: validateImportSchema missing content rejected: " + e.message);
  }
